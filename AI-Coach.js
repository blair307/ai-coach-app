...
async function callAI(message) {
    ...
    // Generate insights from this conversation
    if (window.insightsEngine) {
        window.insightsEngine.generateNewInsight(message, data.response);
    }
...
// DYNAMIC INSIGHTS SYSTEM - COPY-PASTE THIS ENTIRE BLOCK
class InsightsEngine {
    constructor() {
        this.backendUrl = 'https://ai-coach-backend-pbse.onrender.com';
        this.insights = [];
        this.loadInsights();
    ...
    async loadInsights() {
        try {
            ...
            const response = await fetch(`${this.backendUrl}/api/insights`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
            ...
            if (response.ok) {
                ...
                await this.generateInsightsFromConversations();
            }
        } catch (error) {
            await this.generateInsightsFromConversations();
        }
    ...
    async generateInsightsFromConversations() {
        try {
            const token = localStorage.getItem('authToken') || localStorage.getItem('eeh_token');
            if (!token) {
                this.showPlaceholderInsights();
            ...
            const response = await fetch(`${this.backendUrl}/api/chat/history`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
                }
            ...
            if (response.ok) {
                ...
                if (messages.length > 2) {
                    const insights = this.analyzeConversations(messages);
                    await this.saveInsightsToBackend(insights);
    ...
    analyzeConversations(messages) {
        ...
        if (topics.stress > 2) {
            insights.push({
                text: "You frequently discuss stress management, showing strong self-awareness about work pressures.",
                createdAt: new Date().toISOString()
            });
        }

        if (topics.leadership > 1) {
            insights.push({
                text: "Leadership development is a recurring theme in your conversations, indicating growth mindset.",
                createdAt: new Date().toISOString()
            });
        }

        if (topics.communication > 1) {
            insights.push({
                text: "You actively seek guidance on communication skills, showing commitment to relationship building.",
                createdAt: new Date().toISOString()
            });
        }
        ...
        if (topics.goals > 1) {
            insights.push({
                text: "Goal-setting appears frequently in your conversations, showing strong future orientation.",
                createdAt: new Date().toISOString()
            });
        }
        ...
        if (insights.length === 0) {
            insights.push({
                text: "You're building a consistent coaching practice by engaging regularly with AI support.",
                createdAt: new Date().toISOString()
            });
        }
    ...
    extractTopics(userMessages) {
        const topics = { stress: 0, leadership: 0, communication: 0, goals: 0 };
        
        userMessages.forEach(message => {
            const content = message.content.toLowerCase();
            if (/stress|overwhelmed|pressure|anxiety/.test(content)) topics.stress++;
            if (/leadership|lead|manage|team/.test(content)) topics.leadership++;
            if (/communicate|conversation|talk|meeting/.test(content)) topics.communication++;
            if (/goal|objective|target|achieve/.test(content)) topics.goals++;
        });

        return topics;
    }

    async saveInsightsToBackend(insights) {
        try {
            const token = localStorage.getItem('authToken') || localStorage.getItem('eeh_token');
            if (!token) return;

            await fetch(`${this.backendUrl}/api/insights`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json'
            ...
            localStorage.setItem('eeh_insights', JSON.stringify(insights));
        }
    }

    async generateNewInsight(userMessage, aiResponse) {
        const content = userMessage.toLowerCase();
        let insight = null;

        if (content.includes('stress') || content.includes('overwhelmed')) {
            insight = {
                text: "You're actively addressing stress - this self-awareness is key to emotional well-being.",
                createdAt: new Date().toISOString()
            };
        } else if (content.includes('team') || content.includes('leadership')) {
            insight = {
                text: "Leadership challenges are important to you - continued focus here will enhance your impact.",
                createdAt: new Date().toISOString()
            };
        } else if (content.includes('goal') || content.includes('plan')) {
            insight = {
                text: "Your goal-oriented thinking demonstrates strong planning and execution capabilities.",
                createdAt: new Date().toISOString()
            };
        }
        ...
        if (insight) {
            ...
            await this.saveInsightsToBackend(this.insights);
            this.updateInsightsDisplay();
        }
    }
    ...
    updateInsightsDisplay() {
        ...
        this.insights.forEach(insight => {
            ...
            const timeAgo = this.formatTimeAgo(insight.createdAt);
    ...
    showPlaceholderInsights() {
        ...
        insightsList.innerHTML = `
            <div class="insight-item">
                ...
                <small>Waiting for conversations...</small>
            </div>
        `;
    }
